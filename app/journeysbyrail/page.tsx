...
export async function generateStaticParams() {
  ...
  // Same Unicode-normalization issue as getPost() in lib/queries.ts, but at
  // the routing layer: Next.js pre-builds a static page file keyed to
  // whatever exact string this returns. If the DB's stored slug is in one
  // Unicode form (composed vs. decomposed accented characters) and the
  // actual link a visitor's browser requests ends up in the other form,
  // Next's routing never finds a matching pre-built page — a 404 that
  // happens before getPost() or any page code ever runs. Normalizing here
  // ensures the pre-built route itself uses the canonical form.
  return posts.map((p) => ({ slug: p.slug.normalize("NFC") }));
}
